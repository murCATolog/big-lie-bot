...
// імпортмуємо модуль fs з NodeJS для роботи файловою системою асинхронно
...
class QuoteBot extends Telegraf {
  ...
  async getUsers(context = null) {
    try {
      ...
      if (context && !this.activeUsers.includes(context.chat.id)) { // фільтр вже існуючих id
        this.activeUsers.push(context.chat.id); // додаємо новий id у об'єкт
        await this.saveUsers(); // записуємо об'єкт у файл
      }
    } catch (error) {
  ...
  async getQuote() {
    try {
      if (this.quotes.length > 0) {
        ...
        const quote = this.quotes[this.quoteIndex].quote.replace(/([_*[\]()~`>#+\-=|{}.!])/g, '\\$1');
        const author = this.quotes[this.quoteIndex].author.replace(/([_*[\]()~`>#+\-=|{}.!])/g, '\\$1');
  ...
  // ОБРОБКА ВСІХ ПОДІЙ
  eventKeys() {
    // інфа при старті бота
    this.start(async (context) => {
      await this.getUsers(context); // запуск метода додавання id чату при старті
      try {
    ...
    this.action('generate', async (context) => {
      ...
      context.reply(await this.getQuote(), { parse_mode: 'MarkdownV2' }, this.getButtonsMenu());
    });
...
// анонімна самовикликаюча функція для запуску бота
(async () => {
  const BigLiesBot = new QuoteBot('YOUR_BOT_TOKEN_HERE');
